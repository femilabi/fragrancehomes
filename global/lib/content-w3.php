...
function make_field($options){
	...
	case 'checkbox':
		$result.='<div class="checkbox">
			<label>
			  <input'.($name? ' name="'.$name.'"': '').($required? ' required': '').($id? ' id="'.$id.'"' : '').($help? ' aria-describedby="'.$id.'-help"' : '').' type="checkbox" '.$attrs_string.($value? ' value="'.$value.'"' : '').'> '. $label.'
			</label>
		  </div>';
		break;
	case 'radio':
...
function make_pager(){
	global $APP;
	include(GLOBAL_PATH.'contents/pager-w3.php');
}
