...
function make_button($text, $url, $type = 'default', $icon = '', $target = '', $attrs = null){
	...
	$result.='>'.(isset($icon) && $icon? '<span class="glyphicon glyphicon-'.$icon.'" aria-hidden="true"></span> ' : '').$text.'</a>';
	return $result;
}
function make_submit($text, $type = 'default', $icon = '', $name = '', $value = '', $add_issubmit = '', $additional_buttons = ''){
	...
	$result.= '<div class="form-group">
		<div class="col-sm-offset-3 col-sm-9 col-md-7">
		  <button type="submit" class="btn'.($type? ' btn-'.$type : '').'"'.($name? ' name="'.$name.'"': '').($value? ' value="'.$value.'"': '').'>'.($icon? '<span class="glyphicon glyphicon-'.$icon.'" aria-hidden="true"></span> ' : '').$text.'</button>';
...
function make_field($options){
	...
	case 'checkbox':
		$result.='<div class="checkbox">
			<label>
			  <input'.($name? ' name="'.$name.'"': '').($required? ' required': '').($id? ' id="'.$id.'"' : '').($help? ' aria-describedby="'.$id.'-help"' : '').' type="checkbox"> '. $label.'
			</label>
		  </div>';
		break;
	case 'radio':
		$result.='<div class="radio">
			<label>
			  <input'.($name? ' name="'.$name.'"': '').($required? ' required': '').($id? ' id="'.$id.'"' : '').($help? ' aria-describedby="'.$id.'-help"' : '').' type="radio"> '. $label.'
			</label>
		  </div>';
		break;
	case 'select':
		$result.='<select class="form-control"'.($name? ' name="'.$name.'"': '').($id? ' id="'.$id.'"' : '').($help? ' aria-describedby="'.$id.'-help"' : '').'>'.$content.'</select>';
		break;
	case 'static':
		$result.='<p class="form-control-static">'.$value.'</p>';
		break;
	case 'textarea':
		$result.='<textarea class="form-control"'.($name? ' name="'.$name.'"': '').($required? ' required': '').($id? ' id="'.$id.'"' : '').($help? ' aria-describedby="'.$id.'-help"' : '').' type="'.$type.'"'.($placeholder? ' placeholder="'.$placeholder.'"' : '').'>'.$value.'</textarea>';
		break;
	default:
		$result.='<input class="form-control"'.($name? ' name="'.$name.'"': '').($required? ' required': '').($id? ' id="'.$id.'"' : '').($help? ' aria-describedby="'.$id.'-help"' : '').' type="'.$type.'"'.($value? ' value="'.$value.'"' : '').($placeholder? ' placeholder="'.$placeholder.'"' : '').'>';
		break;
	}
...
function print_thumbs($obj, $span = 3, $one_row_scrollable = false){
	...
	for($i = 0, $j = 1; $i < count($obj); $i++, $j++){
		if($j == 1) $result .= '<ul class="thumb row-fluid">';
