...
// Headers
        $headers = 'From:XM Events Ltd <connect@xmeventsng.com>' . $rn;
        $headers .= 'Return-Path:XM Events Ltd <connect@xmeventsng.com>' . $rn;
        $headers .= 'Reply-to:XM Events Ltd <connect@xmeventsng.com>' . $rn;
        $headers .= 'X-Mailer: PHP v'.phpversion() . $rn;
        $headers .= 'X-Originating-IP: '. $_SERVER['SERVER_ADDR'] . $rn;
        $headers .= 'Mime-Version: 1.0' . $rn;
        $headers .= 'Content-Type: multipart/related;boundary=' . $boundary . $rn;
        $headers .= 'Date: '. date('r', $_SERVER['REQUEST_TIME']) . $rn;
        $headers .= 'Message-ID: <'. $_SERVER['REQUEST_TIME']. md5($_SERVER['REQUEST_TIME']). '@'.$_SERVER['SERVER_NAME'] . $rn;
...
// Function mail()
        return mail($to, $subject, $msg, $headers, '-fconnect@xmeventsng.com');
    }
