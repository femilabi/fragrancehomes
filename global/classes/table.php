...
class Table
{
	...
	private function sort_link($key, $text)
	{
		...
		return '<a' . ($this->ajax_src ? ' data-ajax-type="json"' : '') . ' href="' . $url . '" data-toggle="tooltip" class="sort-icon" title="Sort by ' . $text . '"><span class="glyphicon glyphicon-sort-by-alphabet' . ($asc ? '' : '-alt') . '"></span></a>';
	}
